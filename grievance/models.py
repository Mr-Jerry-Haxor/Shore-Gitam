...
issue_types = (("payment", "Pending Payment Issue"),)
...
class Ticket(models.Model):
    ...
    status = models.CharField(
        choices=issue_status, max_length=255, default=issue_status[0][0]
    )
    utr = models.CharField(max_length=255, unique=True)
    remark = models.TextField(max_length=1000, blank=True, null=True)
    attached_documents = models.FileField(
        upload_to=documents_upload_to, null=True, blank=True
    )
