...
def security_home(request):
    if (
        ...
        or security_staff.objects.filter(email_id=request.user.email).exists()
    ):
...
def festpass_scan(request):
    if (
        security_staff.objects.filter(email_id=request.user.email).exists()
        and security_staff.objects.get(email_id=request.user.email).is_fest
    ):
        if request.method == "POST":
            ...
            if Student.objects.filter(passhash=qrhash).exists():
                ...
                messages.error(
                    request, "This student has not registered for the festpass."
                )
        ...
        if request.method == "POST":
            ...
            if Fest_status.objects.filter(email=email).exists():
                ...
                if status == "IN":
                    ...
                    adding = Fest_entries(
                        email=email,
                        fullname=Fest_status.objects.get(email=email).fullname,
...
def maingate_scan(request):
    if (
        security_staff.objects.filter(email_id=request.user.email).exists()
        and security_staff.objects.get(email_id=request.user.email).is_main_gate
    ):
        if request.method == "POST":
            qrhash = request.POST.get("qrhash")
        ...
        if request.method == "POST":
            ...
            if Maingate_status.objects.filter(email=email).exists():
                ...
                if status == "IN":
                    ...
                    adding = Maingate_entries(
                        email=email,
                        fullname=Maingate_status.objects.get(email=email).fullname,
                        verifiedby=request.user.email,
                        status="OUT",
                    )
                    adding.save()
                ...
                studentname = student.name
                Feststatus = Maingate_status(
                    email=email, fullname=studentname, status="IN"
                )
                Feststatus.save()
                adding = Maingate_entries(
                    email=email,
...
def coke_scan(request):
    if (
        security_staff.objects.filter(email_id=request.user.email).exists()
        and security_staff.objects.get(email_id=request.user.email).is_coke
    ):
        if request.method == "POST":
            qrhash = request.POST.get("qrhash")
            if Student.objects.filter(passhash=qrhash).exists():
                ...
                if Fest_status.objects.filter(email=email).exists():
                    if coke_list.objects.filter(email=email).exists():
                        ...
                        c = coke_list(email=email, status="Taken")
                        c.save()
                        messages.success(
                            request, "This student has successfully Grabbed coke."
                        )
                        return render(request, "securitycoco.html")
                else:
                    messages.error(
                        request, "This student has not scanned for festpass."
                    )
                    return redirect("cokescan")
            else:
                messages.error(
