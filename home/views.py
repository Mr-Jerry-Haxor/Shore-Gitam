...
def send_ng_email(user_emails):
    ...
    msg = EmailMultiAlternatives(subject, "", from_email, user_emails)
...
def send_guidelines():
    ...
    send_email_async(emails, send_ng_email)
...
def send_festpass_email(user_email):
    ...
    from_email = settings.EMAIL_HOST_USER
    html_content = get_template("home/festpass_email.html").render({"user": user})
...
def send_payment_failed_email(user_email):
    ...
    html_content = get_template("home/payment_issue.html").render(
        {
            "user": user,
            "payment": FestPass.objects.filter(email=user_email)
...
def is_transaction_success(user_email):
    ...
    if user.is_gitamite:
        ...
        if Registrations.objects.filter(email=user_email).exists():
            user_transactions = Registrations.objects.filter(email=user_email)
        elif FestPass.objects.filter(email=user_email).exists():
            user_transactions = FestPass.objects.filter(email=user_email)
        else:
            return False
...
def signup(request):
    ...
    if request.POST:
        ...
        if not any(char.isupper() for char in password):
            messages.error(
                request, "Password should contain atleast 1 uppercase character"
            )
            return redirect("home:signup")

        if not any(char.islower() for char in password):
            messages.error(
                request, "Password should contain atleast 1 lowercase character"
            )
            return redirect("home:signup")
        ...
        if not any(char in "!@#$%^&*()_+-=[]{}|;:,.<>?/" for char in password):
            messages.error(
                request, "Password should contain atleast 1 special character"
            )
            return redirect("home:signup")
...
def festpass(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.GET:
            ...
            if (
                "terms1" not in request.POST
                or "terms2" not in request.POST
                or "terms3" not in request.POST
            ):
                messages.error(
                    request,
                    "Please accept all the terms and conditions to purchase festpass",
                )
            ...
            important_fields = [
                ...
                user.course,
                user.year_of_study,
                user.campus if user.is_gitamite else "non gitamite",
            ]
    ...
    if request.user.is_authenticated:
        ...
        if request.user.is_festpass_purchased:
            if request.user.is_gitamite:
                ...
                elif FestPass.objects.filter(email=request.user.email).exists():
                    user_transactions = FestPass.objects.filter(
                        email=request.user.email
                    )
                    y_count = user_transactions.filter(transaction_status="Y").count()
...
def dashboard(request):
    if request.user.is_authenticated:
        ...
        if Volunteer.objects.filter(email=request.user.email).exists():
            ...
            if volunteer_obj.isvolunteer:
                context["isVolunteer"] = True
            else:
        ...
        if (
            request.user.passhash
            and not request.user.is_festpass_purchased
            and is_transaction_success(request.user.email)
        ):
        ...
        if request.user.is_superuser or any(
            getattr(request.user, field, False) for field in fields_to_check
        ):
            context["isCoreMember"] = True
        else:
            context["isCoreMember"] = False

        if request.user.is_festpass_purchased and is_transaction_success(
            request.user.email
        ):
            context["festpass_validated"] = True
